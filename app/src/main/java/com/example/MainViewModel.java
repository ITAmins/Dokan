...
public class MainViewModel extends AndroidViewModel {
    ...
    public void setDateToToday() {
        this.activeCalendar.setTime(new Date());
        updateActiveDateState();
    }
    ...
    public String generateRuledNotebookReport() {
        ...
        report.append("খরচ ও মাল কেনা বিবরণ:\n");
        report.append("-----------------------------\n");
        List<ExpenseModel> currentExpenses = this.expenses.getValue();
        if (currentExpenses == null || currentExpenses.isEmpty()) {
            ...
            for (ExpenseModel exp : currentExpenses) {
                String prefix = exp.isPurchase() ? "[মাল ক্রয়] " : "[দোকান খরচ] ";
        ...
        report.append("=============================\n");
        report.append("শতকরা লাভ হিসাব (বিক্রির ওপর)\n");
        report.append("-----------------------------\n");
        report.append("মোট বিক্রি: ৳ ").append(PdfExporter.formatBengaliNumber(summary.totalSales)).append("\n");
        report.append("শতকরা লাভের হার: ").append(PdfExporter.formatBengaliNumber((int) Math.round(summary.estimatedGrossMarginRate * 100))).append("%\n");
        report.append("আনুমানিক লাভ: ৳ ").append(PdfExporter.formatBengaliNumber(summary.estimatedGrossProfit)).append("\n");
        report.append("(নোট: মাল কেনা ও ক্যাশ তহবিল ব্যবসার নিজস্ব অংশ)\n");
        report.append("-----------------------------\n");
        ...
        if (summary.cashDiscrepancy != 0.0) {
            if (summary.cashDiscrepancy > 0) {
                report.append("ক্যাশ বাড়তি: +৳ ").append(PdfExporter.formatBengaliNumber(summary.cashDiscrepancy)).append("\n");
            } else {
                report.append("ক্যাশ কম/শর্ট: -৳ ").append(PdfExporter.formatBengaliNumber(Math.abs(summary.cashDiscrepancy))).append("\n");
            }
        }
    ...
    public List<DaySummary> getSummariesForPeriod(String periodType) {
        List<DaySummary> all = getHistoricalSummaries();
        if (all == null || all.isEmpty()) {
            return new ArrayList<>();
        }
        if ("ALL".equalsIgnoreCase(periodType)) {
            return all;
        }

        List<DaySummary> filtered = new ArrayList<>();
        SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy", Locale.US);
        Date now = new Date();
        ...
        String todayKey = getActiveDateKey();

        for (DaySummary ds : all) {
            if ("TODAY".equalsIgnoreCase(periodType)) {
                if (todayKey.equals(ds.dateKey)) {
                    filtered.add(ds);
                }
            } else if ("WEEK".equalsIgnoreCase(periodType)) {
                try {
                    Date d = sdf.parse(ds.dateKey);
                    if (d != null) {
                        long diffMillis = now.getTime() - d.getTime();
